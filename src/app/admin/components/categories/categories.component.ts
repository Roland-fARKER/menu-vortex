...
export class CategoriesComponent {
  ...
  onSubmit(): void {
    if (this.categoriaForm.valid) {
      ...
      if (this.isEditing && this.currentCategoryId !== null) {
        this.categoriasService.actualizarCategoria(this.currentCategoryId, categoriaData)
          .then(() => {
            this.closeForm();
          });
      } else {
  ...
  deleteCategoria(id: string): void {
    ...
    .obtenerProductos()
    .pipe(map((productos: Producto[]) => productos.filter((p) => p.categoria === id)));

  productosEnCategoria.subscribe((productos) => {
    if (productos.length > 0) {
      alert(`No se puede eliminar esta categoría porque hay ${productos.length} productos asociados.`);
      return;
    }
  ...
  getProductCountByCategory(categoryId: string): number {
    let count = 0;
    this.productosService.obtenerProductos().subscribe((productos) => {
      count = productos.filter((producto) => producto.categoria === categoryId).length;
    });
    return count;
  }
}
